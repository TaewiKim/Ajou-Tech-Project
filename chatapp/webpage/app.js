...
(() => {
  ...
  const T_STORE = "chatgpt_clone_conversations_v1";
  ...
  function loadConversations() {
    const raw = localStorage.getItem(T_STORE);
    if (!raw) return [];
    try { return JSON.parse(raw); } catch { return []; }
  }
  function saveConversations() {
    localStorage.setItem(T_STORE, JSON.stringify(conversations));
  }
  ...
  function deleteConversation(id) {
    ...
    if (!conversations.length) { saveConversations(); createConversation(); return; }
  ...
  function renameConversation(id, title) {
    ...
    c.title = title; saveConversations(); renderConversations();
  }
  ...
  el.composerForm.addEventListener("submit", async (e) => {
    ...
    try {
      const { answer } = await window.Api.chat(prompt, conv.id, abortController.signal);
  ...
  el.clearAllBtn.addEventListener("click", () => {
    if (confirm("모든 대화를 삭제할까요?")) { conversations = []; saveConversations(); createConversation(); }
  });
  ...
  el.promptInput.addEventListener("keydown", (e) => {
    if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); el.composerForm.requestSubmit(); }
  });
  ...
  function applyAuthUI() {
    ...
    if (loggedIn && t?.id_claims) {
      const name = t.id_claims.name || t.id_claims["cognito:username"] || "사용자";
